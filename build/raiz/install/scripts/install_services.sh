...
if  [ -f ../scripts/polo ]; then
	...
	sed -i 's/172.12.0.2/'"172.12.0.3"'/g' ../../data/moodle/public/config.php
	sed -i 's/SCRIPT/'"avapolos_sync_polo.sh"'/g' ../../data/moodle/public/admin/tool/avapolos/view/functions.php
	#cp sync/sincro.php ../../data/moodle/public/admin/tool/avapolos/view/
	cp sync/variables.sh ../../
	sed -i 's/SERVER/'"$ip"'/g' ../../data/moodle/public/admin/tool/avapolos/view/sincro.php
	sed -i 's/avapolos_sync_ies.sh/'"avapolos_sync_polo.sh"'/g' ../../data/moodle/public/admin/tool/avapolos/view/functions.php
	sed -i 's/INSTANCENAME/'"POLO"'/g' ../../variables.sh

	#------------CHAVES-------------

	chown $uid:docker -R /opt/AVAPolos
	sudo -i -u avapolos bash /home/avapolos/installPrivKey.sh

	#------------CHAVES-------------

else
	cd /opt/AVAPolos
	sed -i 's/SCRIPT/'"avapolos_sync_ies.sh"'/g' $working_dir/../../data/moodle/public/admin/tool/avapolos/view/functions.php
	sed -i 's/SERVER/'"$ip"'/g' $working_dir/../../data/moodle/public/admin/tool/avapolos/view/sincro.php

	#------------CHAVES-------------

	chown $uid:docker -R /opt/AVAPolos
	sudo -i -u avapolos bash /home/avapolos/generatePrivKey.sh
